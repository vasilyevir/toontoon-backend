...
async def test_a_job_that_worked_adds_no_complaints(база):
    ...
    # Сравниваем ВИД жалобы, а не строку целиком: в неё вписаны счётчики
    # («10 из 37»), и любая новая работа их сдвигает — та же самая жалоба
    # выглядит новой. Ловушка была тихой: пока отказов за сутки не набиралось,
    # жалобы не было ни до, ни после, и подмену никто не замечал. Стоило
    # провайдерам лечь по-настоящему — тест упал на том, что жалоба осталась
    # прежней, а числа в скобках стали другими.
    # Убираем ВСЕ числа, а не только скобки. Первая попытка вычёркивала
    # «(10 из 37)» и на том успокоилась — но счётчик вшит и в саму фразу:
    # «каждая 4-я работа» после одной удачной становится «каждой 5-й». Та же
    # жалоба, другие слова, и разность множеств снова не пуста.
    без_чисел = lambda жалобы: {re.sub(r"\d+", "#", re.sub(r"\s*\([^)]*\)", "", ж))
                                for ж in жалобы}
    прибавилось = без_чисел(стало.reasons) - без_чисел(было)
...
# ─── что видно в тревоге ─────────────────────────────────────────────────────
#
# Тревога говорила «каждая 4-я работа не получается» и молчала о том, где беда.
# Разбираться приходилось руками — а бед в тот час было сразу две и разные:
# OpenRouter отвечал «кончились деньги», fal рисовал кадр и не мог его отдать.
# Общая доля сливает их в одно число, по которому не понять, звать ли человека
# с картой или чинить сеть.

ЦЕПОЧКИ = [
    (
        'GenerationUnavailable("All providers failed for image_to_image: '
        "fal_nano_edit: GenerationUnavailable('fal: кадр не скачался за 30 с с "
        "v3b.fal.media — хранилище fal отдаёт слишком медленно (ReadTimeout)')\")",
        ["fal_nano_edit", "не скачался", "fal.media"],
    ),
    (
        'GenerationUnavailable("All providers failed for image_to_image: '
        "openrouter_gemini_pro: GenerationUnavailable('OpenRouter HTTP 402: "
        '{"error":{"message":"Insufficient credits."}}\')")',
        ["openrouter_gemini_pro", "402", "Insufficient credits"],
    ),
    ("Работа оборвалась: процесс не дожил до конца (найдена сверкой).",
     ["оборвалась"]),
]


@pytest.mark.parametrize("цепочка,ждём", ЦЕПОЧКИ)
def test_из_цепочки_видно_кто_и_почему(цепочка, ждём):
    коротко = diagnosis.коротко(цепочка)
    for кусок in ждём:
        assert кусок in коротко, f"«{кусок}» потерялось: {коротко}"


def test_обёртки_не_съедают_строку():
    """Раньше девяносто знаков уходили на обёртки и строка кончалась на «GenerationU»."""
    коротко = diagnosis.коротко(ЦЕПОЧКИ[0][0])
    assert not коротко.startswith("GenerationUnavailable")
    assert "All providers failed" not in коротко


def test_пустая_причина_не_роняет():
    assert diagnosis.коротко(None)
    assert diagnosis.коротко("")
