...
async def _generate(body: GenerateRequest, ctx: Context, db: AsyncSession) -> GenerateResponse:
    ...
    try:
        ...
        задача = dict(
            ...
            # Свою прошлую работу не проверяем «а не фотография ли это»: носитель
            # задаёт исходник, а не стиль, угаданный по тексту просьбы. Иначе правка
            # фотографического кадра («тот же кадр, но закат на фоне») считалась
            # браком и уходила на перерисовку с нуля — цветным пляжем в другой одежде.
