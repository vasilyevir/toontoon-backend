...
class GenerateRequest(BaseModel):
    ...
    # Откуда взялся снимок: `photo_library`, `camera`, `creations`,
    # `ai_profile`. На кадр не влияет — нужно событиям о судьбе работы, которые
    # шлёт сервер. Источник знает только приложение: сервер видит ссылку на
    # файл, а по ней не понять, сняли его сейчас или достали из галереи.
    photo_source: Optional[str] = Field(default=None, max_length=32)
    ...
    # Чем этот заказ отличается от повтора того же самого.
    #
    # Приложение придумывает ключ в момент, когда человек решился, и присылает
    # ТОТ ЖЕ ключ, если пришлось послать запрос заново — не дождавшись ответа на
    # плохой связи. Сервер тогда отдаёт прежний заказ вместо второго списания.
    #
    # Пусто — старая сборка приложения. Такой запрос обрабатывается как раньше:
    # ломать работающих людей ради защиты, которую они не умеют попросить,
    # нельзя.
    ...
    from_chat: bool = True
    # Чей профиль подставлять, если снимок не приложен.
    #
    # Пусто — основной. Поле нужно там, где человек переключился на другого:
    # «сделай Аню на постере» должно взять её набор, а не его.
    profile_id: Optional[str] = None
    # Кого рисуем, когда людей несколько: «нас с Аней».
    #
    # Список, а не одно поле, потому что совместный кадр — не набор из двух
    # генераций, а одна: обоих надо привести в один кадр за одно списание.
    # Порядок значим — он же станет порядком референсов, а по нему модель и
    # понимает, кто из них кто.
    profile_ids: list[str] = Field(default_factory=list)
    # Записать просьбу в переписку самим.
    #
    # Обычно текст уже уехал туда через `/api/chat`. Но не всегда: в режиме «как
    # сказано» разговора нет вовсе, а нажатие на готовую идею — это выбор, а не
    # реплика. И то и другое человек видел на экране, а назавтра не находил:
    # кадр в треде был, а просьбы, по которой он сделан, не было.
    post_prompt: bool = False
