...
class ContentRefused(GenerationUnavailable):
    """Исполнитель отказался по СОДЕРЖАНИЮ, а не по обстоятельствам.

    Это не поломка, это решение — и оно окончательное. Реестр обходит отказы,
    чтобы одна плохая минута у вендора не стоила человеку кадра: квота, 5xx,
    оборвавшееся соединение. Отказ по содержанию из этого ряда выпадает:
    обойти его значит пойти к тому исполнителю, который не проверяет, — и тем
    самым сознательно снять защиту, которую нам дали бесплатно.

    Проверено на живом примере: OpenAI отклонил «сделай меня как Том Круз» за
    пять секунд, а nano-banana такой же запрос нарисовал. Пока отказ по
    содержанию ничем не отличался от таймаута, цепочка делала ровно это.
    """
