...
# Кому уходит ПОВТОР, когда рисованный кадр вернулся фотографией.
#
# Раньше повтор шёл к тому же исполнителю, который только что провалился, — с
# теми же словами, только строже. Замер 26 августа на одном и том же промпте,
# по три кадра:
#
#     gemini-3-pro     рисунком 2/3   с буквами 2/2   $0.1377
#     gemini-3.1-flash рисунком 3/3   с буквами 2/3   $0.0681
#     gpt-image-2      рисунком 3/3   с буквами 0/3   $0.0472
#
# Pro остаётся основным: он единственный надёжно набирает буквы, а постер без
# надписи — не постер. Но срывается он примерно в трети случаев, и второй заход
# к нему же — это ставка на ту же монету. Flash рисует безотказно и буквы всё
# же ставит; gpt-image-2 рисует лучше всех и не ставит их вовсе, поэтому в
# запасные не годится.
REDRAW_FALLBACK_PROVIDER = "openrouter_gemini"
