...
class Settings(BaseSettings):
    ...
    #
    # Три — но только на фотографическом пути, где тяга к фотографии и нужна.
    # Рисованному кадру три снимка вредят: тот же постер с тремя референсами
    # дважды пришёл фотографией, с одним — дважды аниме-постером. Решает это
    # `_reference_take`, здесь только потолок.
    #
    # Улыбку, ради которой цифру и поднимали, снял не счётчик, а отдельная
    # оговорка в промпте: выражение приходит из сцены, а не с референса.
    #
