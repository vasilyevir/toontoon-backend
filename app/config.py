...
class Settings(BaseSettings):
    ...
    # Кадр из разговора — 10 монет, как и любой кадр из каталога.
    #
    # Раньше стояло 15: «середина» между фото за 10 и рисунком за 20, когда
    # рисунки шли к Gemini Pro вдвое дороже. С 7 сентября 2026 все кадры рисует
    # gpt-image-2 через fal по одной цене (замер: $0.0445 за кадр, рисованный с
    # перерисовками ≈ ×1.3, а не ×2), и рисованные стили в каталоге тоже стоят
    # 10. Держать разговор дороже каталога значило бы брать за то же самое
    # больше только потому, что человек описал сцену словами.
    image_toontoon_cost: int = 10
