...
class Settings(BaseSettings):
    ...
    # Сколько пикселей разрешено разобрать из присланного файла.
    #
    # Потолок в байтах (15 МБ) не защищает от PNG: 121 мегапикселя одного
    # цвета весят 366 КБ, а распакованные — около 900 МБ. Pillow отказывает
    # только выше 2× своего лимита, между 1× и 2× лишь предупреждает, и на
    # каждую параллельную загрузку уходит по гигабайту. Двадцать четыре
    # мегапикселя — больше любого кадра, который мы отдаём (1,5 Мп), и
    # достаточно для снимка с телефона (12 Мп); 48-мегапиксельный HEIC
    # телефон и так пережимает перед отправкой.
    max_image_pixels: int = 24_000_000
    # Генераций в час на человека. Поле потерялось в ad6c072 при правке цен,
    # и /api/generate отвечал 500 на каждый запрос — тесты этого не ловили,
    # потому что до лимита в них никто не доходил.
    rate_limit_per_hour: int = 30

    # Как часто сверять долги перед людьми: работы, за которые взяли деньги, а
    # кадра не дали. Раньше только на старте процесса — то есть возврат ждал
    # следующей выкатки. Ноль выключает расписание (сверка на старте остаётся).
    refund_sweep_interval_seconds: int = 300

    # ── Политика контента (app/services/policy.py) ──────────────────────────
    # Выключатель на случай, если проверка начнёт отказывать всем подряд:
    # модерация провайдера остаётся и без неё.
    policy_enabled: bool = True
    # Направления каталога, в которых допустим несовершеннолетний в кадре.
    # Остальные — телесные или гламурные, и ребёнку там не место.
    minor_safe_categories: str = (
        "ai_photo_studio,black_and_white,cartoon_me,polaroid_reunion,family_fun,pet_magic"
    )
    # Сколько отказов по политике (наших или провайдера) в сутки закрывают
    # человеку генерацию до конца суток.
    policy_refusals_per_day: int = 5
    # Куда писать, если проверка приняла человека за публичное лицо, а это он сам.
    ...
    # Пять — ровно треть кадра, и это сделано нарочно (Илья, 15 сентября 2026).
    #
    # Пятёрка на входе и десятка первой ежедневной награды дают 15 — цену
    # одного кадра, ни монетой больше. Человек без подписки делает первую
    # работу бесплатно, но чтобы её сделать, ему надо дождаться награды и
    # забрать её: подарок, за которым потянулся сам, ощущается иначе, чем
    # подарок, выданный молча. До этого на входе лежала десятка, награда
    # добавляла вторую, и после первого кадра оставалось пять — остаток,
    # которым нельзя воспользоваться.
    signup_toontoon_balance: int = 5
    ...
    # ── Набор дня (вкладка Shots) ────────────────────────────────────────────
    #
    # Тридцать карточек: шесть занимали пол-экрана и кончались на первом же
    # движении пальца, а вкладка называется набором дня, а не парой примеров
    # (Илья, 15 сентября 2026).
    shots_per_day: int = 30
    # Закреплённая голова набора — четыре карточки, которые не крутятся.
    #
    # Порядок читается парами: девушка и она же в мультфильме, парень и он же
    # в мультфильме. По этим четырём человек за секунду понимает, что здесь
    # делают и на ком это работает; отдавать первое впечатление остатку от
    # деления по дате незачем.
    #
    # Sunlit (`warm_knit`) выбран Ильёй прямо на витрине.
    shots_pinned_ids: str = "luxury_rooftop,plant_room,warm_knit,bedroom_beats"

    @property
    def shots_pinned(self) -> list[str]:
        return [s.strip() for s in self.shots_pinned_ids.split(",") if s.strip()]
    ...
    # Где рисуется кадр. `inline` — фоновой задачей внутри веб-процесса, как
    # было всегда: просто, но деплой или падение пода убивает всё, что рисуется,
    # и человек ждёт «готовится» до stale_generation_minutes. `queue` — заказ
    # кладётся в Redis-очередь, рисует отдельный процесс `python -m app.worker`,
    # который при старте подбирает всё недорисованное. Умолчание — прежнее:
    # включать сознательно, вместе с воркером в чарте.
    worker_mode: str = "inline"
    worker_queue_key: str = "toontoon:jobs"
    # Сколько кадров воркер рисует одновременно. Рисование — это ожидание сети
    # (fal держит ≥ 6 заказов разом, замер 2026-09-02), и один кадр за раз
    # превратил бы восьмерых людей в очередь на пять минут. Четыре — с запасом
    # под лимит fal и под CPU обработки кадра (Pillow в пуле потоков).
    ...
    #
    # Одна модель — одна цена: с 7 сентября 2026 все кадры рисует gpt-image-2
    # через fal ($0.0445 за кадр по счёту fal, рисованный с перерисовками ≈ ×1.3),
    # и разговор стоит столько же, сколько каталог. 15 вместо 10 — решение
    # 8 сентября 2026 вместе с новыми тарифами (Weekly $9.99 / 700, Yearly $59.99).
    image_toontoon_cost: int = 15
    # Эксперимент (Илья, 2026-09-09): одежда — с витринного кадра стиля, а не
    # с фотографии человека. Кадр стиля прикладывается к заказу последним
    # референсом, промпт просит взять с него одежду и подогнать под пол и
    # сложение человека. Выключено по умолчанию; ветка exp/style-wardrobe.
    style_wardrobe_from_example: bool = False
    ...
    # Сколько всего ждать кадр, включая очередь. Было девяносто — как у
    # OpenRouter. Стало сто пятьдесят, потому что основной исполнитель
    # сменился: gpt-image-2 в `medium` честно работает 34–46 с (замер
    # 2026-09-02, 864×1536 и 1152×1536), и девяносто оставляли всего двойной
    # запас на очередь fal — а очередь под нагрузкой считается против этого же
    # срока. `high` (81 с) тоже влезает, хотя мы его не берём.
    # Дольше нельзя: человек ждёт с экраном «готовится», и после двух минут
    # уходит — а кадр потом всё равно оплачен.
    fal_request_timeout: float = 150.0
    # Сколько ждать В ОЧЕРЕДИ fal, отдельно от самой работы. Под нагрузкой заказ
    # может стоять в очереди дольше, чем рисуется, и считать это против того же
    # срока значит массово отказывать (с возвратами) при живом провайдере. Очередь
    # ждём своё, работу — своё; сумма ограничена fal_request_timeout.
    fal_queue_timeout: float = 120.0
    ...
    # Докачка по кускам — для моделей, которые вернули ссылку вместо кадра.
    #
    # Хранилище fal (`v3b.fal.media`) с нашего пути отдаёт ~16 КБ на TCP-
    # соединение и замолкает; новое соединение получает ещё 16 КБ с любого
    # смещения (Range поддержан). Полтора мегабайта так доезжают за ~66 с и
    # 92 соединения — вместо гарантированного отказа. Размер куска взят из
    # замера; таймаут — на КУСОК, а не на файл, иначе стену видно лишь через
    # полминуты.
    fal_chunk_bytes: int = 16 * 1024
    fal_chunk_timeout: float = 10.0
    fal_chunk_retries: int = 3
    fal_download_deadline: float = 180.0
    # Ссылку на кадр присылает fal, и верим мы ей ровно настолько: только
    # https и только их хранилище. Подмена `FAL_BASE_URL` или перехват ответа
    # не должны превращать докачку в запрос к произвольному адресу.
    fal_download_hosts: str = "fal.media,fal.run,fal.ai"
    # Кадр — единицы мегабайт; 64 МБ это не картинка, а попытка занять память.
    fal_download_max_bytes: int = 64 * 1024 * 1024
    ...
    # Запасной путь для текста — через fal.
    #
    # fal своих языковых моделей не держит: `openrouter/router` проксирует тот
    # же OpenRouter, только платим мы фаловскими кредитами. Это и ценно: ключ
