...
class Settings(BaseSettings):
    ...
    #
    # Три. Один снимок диктует не только лицо, но и мину: набор почти всегда
    # начинается с улыбки — так люди фотографируются, — и человек получал одну
    # и ту же улыбку в любой сцене, хоть под дождём, хоть на драматичном
    # постере. Три кадра с разными выражениями снимают этот диктат, а порядок
    # отбора остаётся за разбором набора.
    #
