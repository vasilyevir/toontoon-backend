# TOONTOON — App-key + HMAC подпись запросов (setup)
...
**Заголовки запроса:**
```
X-Toontoon-App-Key:   <APP_KEY>
X-Toontoon-Timestamp: <unix seconds>
X-Toontoon-Signature: <hex hmac-sha256>
```

Клиент: `ToontoonApp/Networking/APISecurity.swift → decorate()`.
...
| Где | Файл | В git? |
|---|---|---|
| Приложение | `Config.xcconfig` → `TOONTOON_APP_KEY` / `TOONTOON_APP_SECRET` | ❌ (git-ignored) |
...
1. Собрать приложение с непустыми `TOONTOON_APP_KEY`/`TOONTOON_APP_SECRET`
