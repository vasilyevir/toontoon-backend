# TOONTOON iOS — Аналитика: полный аудит и план внедрения

Большой аудит: что можно собирать, **что пройдёт ревью Apple**, какие SDK взять,
какие события трекать именно в TOONTOON, как внедрить и что запрещено.
...
## 1. Что хотим понять о поведении (цели аналитики TOONTOON)
...
- **Экономика TOONTOON:** сколько тратят, на что, упираются ли в баланс (402).
...
- **Вывод для TOONTOON:** делаем first-party аналитику → **ATT-prompt не нужен**,
...
- **Дети/чувствительное**: не собирать спец-категории; у TOONTOON аудитория 40–60,
...
## 6. Каталог событий TOONTOON (собираем максимум легально)
...
- `generation_failed` {type, tile_id, reason: insufficient_toontoon_402|rate_limit_429|generator_503|timeout|network, duration_ms}
...
- `toontoon_spent` {amount, reason: image_generate|video_generate, balance_after}
- `insufficient_toontoon` {needed, balance}
...
toontoon_balance_bucket, install_date, days_since_install.
...
видео, TOONTOON-экономика, D1/D7/D30 retention, топ тайлов/категорий, error rate по эндпоинтам.
...
- `ToontoonApp.init` авто-выбирает TelemetryDeck, если пакет добавлен И задан App ID.
- Config: `TOONTOON_TELEMETRYDECK_APP_ID` (Info.plist ← Config.xcconfig).
...
```
TOONTOON_TELEMETRYDECK_APP_ID = <твой-app-id>
```
Готово: события (запуск, вход/регистрация, воронка генерации, трата TOONTOON)
