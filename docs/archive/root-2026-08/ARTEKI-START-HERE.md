# ARTEKI iOS — START HERE (с чего начать)
...
(`intersson-sir/arteki-ios`), секреты — нет (см. §5).

---

## 1. TL;DR — что это и как устроено

- **Что:** нативное iOS-приложение (SwiftUI, min iOS 16) для ARTEKI — генератор
...
### Шаг 0. Забрать код
```bash
git clone https://github.com/intersson-sir/arteki-ios.git
cd arteki-ios
```
...
2. Удали автосозданные `ContentView.swift`/`App.swift` — используем свои из
   `ArtekiApp/`.
3. Перетащи папку **`ArtekiApp/`** в проект (Create groups, добавить в таргет).
...
2. Все картинки из `Resources/Images/` (assets/ + landing/) → добавить в
   **Assets.xcassets** (имена без расширения = как в коде: `icon-token`,
   `Userpic`, `arteki-logo`, `Close` и т.д.).
...
- Весь Swift-код `ArtekiApp/`, все ассеты `Resources/Images/`, `PrivacyInfo.xcprivacy`.
