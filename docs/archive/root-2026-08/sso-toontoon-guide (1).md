...
Резерв живёт **5 минут**. Если не подтверждён и не отменён — при следующей попытке confirm вернёт `payment_expired` (статус `failed`), а зарезервированный cap вернётся в лимит гранта.
