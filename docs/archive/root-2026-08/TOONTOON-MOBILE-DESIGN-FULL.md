# TOONTOON — полная мобильная дизайн-спецификация (1:1 с вебом)
...
Логотип/иконки: `toontoon-logo.svg`, `icon-token.svg`, `icon-new-chat.svg`,
...
h52, `px14`, `toontoon-logo.svg` 82×40); справа `TopNavRight mobile` (см. §8).

**Центр (колонка, gap 20, maxWidth 440):**
1. Пилюля-анонс: h44, бордер `#474747`, capsule; `freepik-avatar.png` 32×32
   (margin 6) + «I'm Toontoon, your assistant» 15/medium белый.
...
- **Логотип-плашка** h52, фон `#141414`, radius 14, `px14`, `toontoon-logo.svg`
  82×40 → ведёт на «/».
...
- **confirming:** аватар AI + текст-пузырь `#222` («Everything is ready! Click to
  ...
  **токен-карточка**: ширина 100% (maxWidth 400), фон `#141414`, radius 24, `p12`,
  ...
  - Иначе — **Let's generate – {cost} TOONTOON**: h64, фон `#8101FF`, radius 16,
    `icon-token` 24 + 18/semibold белый.
  - Под кнопкой строка баланса: `icon-token` 24 + «{tokens} TOONTOON» 14/medium белый.
...
4. confirming → `POST /api/generate` (тело: `tile_id`+`answers`, либо `prompt`+`style`).
   ...
   - Видео: ответ `status:queued` → polling `GET /api/generations/{id}` каждые ~5–6с
     ...
     (TOONTOON возвращается автоматически).
5. Баланс обновляется из ответа `/generate` (`balance`) и `GET /api/balance`.
Стоимость: картинка/открытка 1 TOONTOON, видео 2 TOONTOON.
...
- **Токен-чип:** фон `#222`, radius 12, `pt4 pb4 pl8 pr4`; `icon-token` 24 +
  «{tokens} TOONTOON» 14/medium белый. (Кнопка Upgrade — `display:none` на мобиле.)
...
- **Credit balance** 16/semibold + чип фон `#755BEF`, radius 12, `p8`,
  `icon-token` 24 + «{tokens} TOONTOON» 16/semibold белый.
...
**Триггер:** только первый заход (нет флага `toontoon_onboarding_done`). После
...
1. `prompt` — **«Describe your idea»** / «Type what you'd like to create here and
   press Generate — Toontoon turns your words into an image.»
