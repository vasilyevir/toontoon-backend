# TOONTOON — Полная документация проекта
...
1. [Что такое TOONTOON](#1-что-такое-toontoon)
...
8. [Кошелёк TOONTOON](#8-кошелёк-toontoon)
...
## 1. Что такое TOONTOON

TOONTOON — это AI-генератор персонализированного визуального контента. Пользователь открывает веб-приложение, выбирает тип контента (изображение, открытка, видео), отвечает на 2–4 быстрых вопроса (Quick Reply кнопки) и получает готовый красивый 3D-мультяшный контент за несколько секунд (изображения) или несколько минут (видео).

**Целевая аудитория:** люди 40–60 лет, не техносавантные, хотят быстро создать открытку или видео-поздравление для близких.

**Единица оплаты:** TOONTOON — внутренняя валюта. При регистрации выдаётся 50 TOONTOON. Изображение стоит 1 TOONTOON, видео — 2 TOONTOON.
...
Чат — это conversational UI, имитирующий мессенджер. Toontoon отвечает текстом, задаёт вопросы и показывает контент прямо в пузырях сообщений. Пузыри бывают:
...
|-----|--------|-----------|-----------------|
| Image | 6 | 1 TOONTOON | 15–45 сек |
| Postcard | 13 | 1 TOONTOON | 15–45 сек |
| Announcement | 7 | 1 TOONTOON | 15–45 сек |
| Video | 8 | 2 TOONTOON | 4–8 мин |
...
- **systemd** — `toontoon-backend.service` и `toontoon-frontend.service` (автостарт)
...
| Ключ | Содержимое |
|------|-----------|
| `toontoon_chat_history` | JSON-массив метаданных чатов (id, label, image) |
| `toontoon_chat_msgs_{id}` | JSON-массив сообщений чата N |
| `toontoon_gen_active` | `{id, label, isVideo, startedAt}` — активная генерация |
| `toontoon_start_category` | Стартовая категория при переходе с лендинга |
| `toontoon_migration_v1_cleaned` | Флаг миграции |
| `toontoon_migration_v2_video_thumbs` | Флаг миграции |
...
2. GET /api/auth/verify?token=...
   ...
   → get_or_create_magic_user(email) — ищет по user:email:{email}
     → если не нашёл: создаёт User с toontoon_balance=50
   → create_session(user) → session:{sid} в Redis (TTL 30 дней)
   → Set-Cookie: toontoon-session={sid}; SameSite=Lax
...
cancel(user, payment):  ← при ошибке генерации
  ├── user.toontoon_balance += payment.amount  (рефанд)
  └── save_user(user)
```

**Стоимость:**
- Изображение / открытка / анонс: **1 TOONTOON** (конфигурируется `IMAGE_TOONTOON_COST`)
- Видео: **2 TOONTOON** (конфигурируется `VIDEO_TOONTOON_COST`)
...
4. cost = tile.cost = 1
5. wallet.reserve(user, amount=1)             # снять 1 TOONTOON (refund on fail)
...
Toontoon — персонаж-ассистент. Использует `gpt-4o-mini` с системным промптом `_CHAT_SYSTEM`.
...
localStorage.setItem("toontoon_gen_active", JSON.stringify({
  id: res.id,         // gen_xxx
...
const saved = JSON.parse(localStorage.getItem("toontoon_gen_active"));
...
localStorage.removeItem("toontoon_gen_active");
```
...
| GET | `/api/balance` | ✓ | Баланс TOONTOON |
...
| Метод | URL | Auth | Описание |
|-------|-----|------|---------|
| POST | `/api/chat` | — | Сообщение Toontoon |
...
| `SIGNUP_TOONTOON_BALANCE` | `50` | Начальный баланс |
...
| `IMAGE_TOONTOON_COST` | `1` | |
| `VIDEO_TOONTOON_COST` | `2` | |
| `VAPID_PRIVATE_KEY` | *** | Web Push |
| `VAPID_PUBLIC_KEY` | *** | Web Push |
| `VAPID_EMAIL` | `mailto:hello@toontoon.ai` | |
...
### Systemd unit: `toontoon-frontend`
...
- Backend: `https://github.com/intersson-sir/toontoon-gen-backend.git`
- Frontend: отдельный репозиторий

### Управление

```bash
systemctl restart toontoon-backend
systemctl restart toontoon-frontend
systemctl status toontoon-backend

journalctl -u toontoon-backend -f    # live logs
```
