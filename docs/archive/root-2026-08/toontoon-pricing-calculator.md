# TOONTOON Pricing Calculator — техническое задание
...
3. курс TOONTOON           (парсинг с биржи azbit → базовый курс)
   ├─ unlock-курс      (для разлоченных токенов)
   └─ lock-курс        (для залоченных токенов)
4. итог в TOONTOON         (retailUsd / курс) — отдельно для unlock и lock
```

Два артефакта:

1. **`src/lib/toontoonPricing.mjs`** — чистый движок (config + функции, без зависимостей от Next).
2. **`scripts/toontoon-cost.mjs`** — CLI: тянет живой курс, печатает таблицу цен.
...
- Парсер курса TOONTOON/USDT с azbit + вывод двух производных курсов (unlock/lock).
- Калькулятор: COGS → ×3 → перевод в TOONTOON по обоим курсам.
...
- Реальное списание TOONTOON у пользователя (флоу Boostify — уже есть отдельно).
...
```json
{
  ...
  "currencyPairCode": "TOONTOON_USDT",
  "price": 17.64487,
...
const toontoon = all.find(t => t.currencyPairCode === "TOONTOON_USDT");
const baseRate = toontoon.price;                // USD за 1 TOONTOON (last price)
```
...
> Рекомендация: для расчёта **себестоимости в TOONTOON** честнее `bid` или `mid`,
...
```
Курс — это **$ за 1 TOONTOON**. Чем курс ниже, тем больше токенов нужно за ту же
...
priceUnlockToontoon = retailUsd / unlockRate
priceLockToontoon   = retailUsd / lockRate
```
...
**Округление:** деньги — 4 знака (`$0.1266`); TOONTOON — настраиваемый
`TOONTOON_DECIMALS` (дефолт 4), плюс отдельно «человеческое» округление для UI
(`TOONTOON_UI_DECIMALS`, дефолт 2). Округлять в большую сторону (`Math.ceil` на
...
| Тип | COGS $ | ×3 retail $ | unlock TOONTOON | lock TOONTOON |
|-----|-------:|------------:|------------:|----------:|
...
```
src/lib/toontoonPricing.mjs     # движок: config + чистые функции + fetch курса
scripts/toontoon-cost.mjs       # CLI: import движка → печать таблицы
```
 ...
 *              source:"last"|"bid"|"mid", stale:boolean, fetchedAt:number }} ToontoonRate */

/** @typedef {{ type:GenType, cogsUsd:number, retailUsd:number,
 *              unlockToontoon:number, lockToontoon:number }} GenPrice */
 ...
 *  @returns {Promise<ToontoonRate>} */
export async function fetchToontoonRate() {}
 ...
 *  @param {GenType} type @param {ToontoonRate} rate @returns {GenPrice} */
export function priceGeneration(type, rate) {}

/** Расчёт всех типов сразу. @param {ToontoonRate} rate @returns {GenPrice[]} */
...
**Альтернатива (если хотим строгий TS):** движок в `src/lib/toontoonPricing.ts`,
CLI через `npx tsx` (добавить `tsx` в `devDependencies` и скрипт
`"toontoon:cost": "tsx scripts/toontoon-cost.ts"`). Минус — новая зависимость. По
...
cd /Users/zhenyashe/XCloude/toontoon
node scripts/toontoon-cost.mjs
```

**Пример вывода (формат — ориентир):**
```
TOONTOON rate (azbit, last)  $17.64  ▼13.6% 24h   [live]
  unlock (+20%)  $21.17 / TOONTOON
  lock   (−30%)  $12.35 / TOONTOON

Тип       COGS $    ×3 $     unlock TOONTOON   lock TOONTOON
image     0.0422    0.1266   0.0060        0.0103
postcard  0.0422    0.1266   0.0060        0.0103
video     0.7500    2.2500   0.1063        0.1822
```
...
- Пары `TOONTOON_USDT` нет в ответе → fallback, `stale:true`.
...
1. **Конфликт двух моделей цены.** Текущая спека = **cost-plus ×3**:
   ...
   «1 генерация = 0.25 unlock-TOONTOON = $5» (привязка к бирже, не к себестоимости).
   ...
   - фиксированная цена в TOONTOON (0.25 unlock / 5 lock), себестоимость — только для контроля маржи.
   > Движок должен поддержать любой: заложить опциональный `PRICE_FLOOR_USD` и
   > возможность задать фикс-цену в TOONTOON поверх расчёта.
2. **Точные `UNLOCK_PREMIUM` / `LOCK_DISCOUNT`** (`+20% / −30%` — черновик).
3. **Какое биржевое поле** считать курсом для списания (`last` / `bid` / `mid`).
4. **Округление TOONTOON для пользователя** (сколько знаков, всегда вверх?).
...
- [ ] `node scripts/toontoon-cost.mjs` печатает таблицу по **живому** курсу с azbit.
...
- [ ] `npm run lint` чистый.
- [ ] Краткий отчёт по формату из `toontoon/CLAUDE.md` (Что сделано / Что изменено / Проверка / Риски).

---

## 11. Контекст репозитория (чтобы не переоткрывать)

- Экономика/конфиги живут в `src/lib/` (`tileConfig.ts` — у тайлов уже есть `priceToontoon`).
- Текущий мок-топап: `src/app/api/toontoon/topup/route.ts` (`MOCK_AMOUNT = 3`, «until Boostify integration»).
- Захардкоженные легаси-цены (НЕ источник правды): `src/app/preview/page.tsx`
  (`price = 0.40`, `toontoonPrice = price / 0.70`), `src/app/api/generations/route.ts` (`costUsd: 0.40`).
