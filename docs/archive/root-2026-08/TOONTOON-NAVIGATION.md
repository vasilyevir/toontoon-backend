...
реализовано в `App/ToontoonApp.swift` (`RootView`) и `Auth/AuthManager.swift`.
...
```
   ...
   не авторизован  ───► │  Landing  (/landing)        │
                        ...
                        │  подтвердить                │
                        └───────────────┬─────────────┘
                     GET devLink → Set-Cookie toontoon-session
                                        ▼
...
- Бэкенд в `GET /api/auth/verify` ставит cookie `toontoon-session` и делает
...
5. Если пришли с Landing/Start с выбранной категорией или готовым промптом
   (в вебе — `toontoon_start_category` / `toontoon_pending_prompt`), они
...
`pickTile → answering (вопросы тайла) → confirming (оплата TOONTOON) → generating →
...
- Повторный запуск приложения: если cookie `toontoon-session` в Keychain жива и
  `GET /auth/me` вернул юзера → сразу **Generate** (минуя Landing/Login).

## 5. Как это уже реализовано в приложении
`RootView` (`App/ToontoonApp.swift`):
```
