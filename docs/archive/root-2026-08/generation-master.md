# Toontoon — Слой генерации (итоговый продукт)

## §0. Обзор
Toontoon генерит контент через чат. **3 категории**: Картинки, Открытки, Видео (Оживление фото —
...
### 1.1 Флоу
**Гейт перед стартом:** авторизация + баланс TOONTOON. Нет логина / нет TOONTOON → CTA на логин / пополнение,
...
TOONTOON списываем **только при финальном success** результата.
...
## §7. Экономика
Картинка / Открытка = **1 TOONTOON** · Видео / Оживление фото = **2 TOONTOON**.
Видео = **2 шага генерации** (кадр + Seedance) → себестоимость и время выше картинки; курс TOONTOON↔$ — по тарифу.
