# TOONTOON iOS — все интеграции и как их подключать
...
- Весь обмен — через `Networking/APIClient.swift` на **`TOONTOON_BASE_URL`**
  (`https://193.149.190.155`, `/api/*`), по HTTPS с **пиннингом** (`APISecurity`).
- Контракт всех ручек — `API.md` + `openapi.json`. Модели — `Models.swift`.
- Данные, которые тянет апка: каталог тайлов, баланс TOONTOON, генерации (image/
...
|---|---|---|---|
| `TOONTOON_BASE_URL` | Адрес сервера | да (есть дефолт) | наш сервер |
| `TOONTOON_PINNED_SPKI` | Пиннинг TLS-сертификата | рекоменд. | `SECURITY.md` (уже вписан) |
| `TOONTOON_APP_KEY` | Заголовок app-key (барьер от ботов) | опц. | придумать, продублировать на сервере |
| `TOONTOON_APP_SECRET` | Подпись запросов (HMAC) | опц. | придумать, продублировать на сервере |
| `TOONTOON_TELEMETRYDECK_APP_ID` | Продуктовая аналитика TelemetryDeck | опц. | telemetrydeck.com |
| `TOONTOON_SENTRY_DSN` | Мониторинг крашей/ошибок | опц. | sentry.io |
...
| **App-key / подпись запросов** | 🟡 клиент готов | задать `TOONTOON_APP_KEY/SECRET` + включить на сервере (см. SECURITY.md §3) |
| **TelemetryDeck** | 🟡 адаптер готов | ① SPM `github.com/TelemetryDeck/SwiftSDK` ② `TOONTOON_TELEMETRYDECK_APP_ID` |
| **Sentry (краши)** | 🟡 адаптер готов | ① SPM `github.com/getsentry/sentry-cocoa` (продукт «Sentry») ② `TOONTOON_SENTRY_DSN` |
...
→ создать app на telemetrydeck.com → `TOONTOON_TELEMETRYDECK_APP_ID = <id>`.
...
(добавить продукт «Sentry») → создать проект на sentry.io → `TOONTOON_SENTRY_DSN =`
(экранировать `//` как `/$()/`). Краши/ошибки поедут в Sentry.

**App-key/подпись:** задать `TOONTOON_APP_KEY` и `TOONTOON_APP_SECRET`, продублировать
