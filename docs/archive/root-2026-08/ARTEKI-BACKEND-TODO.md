# ARTEKI iOS — чего не хватает НА СЕРВЕРЕ (ТЗ для бэкенда)
...
| `/transactions` | GET | ✅ (401 без токена) | история TEKI |
...
1. Домен (напр. `api.arteki.ai`) → A-запись на `193.149.190.155`.
2. Let's Encrypt (certbot) вместо self-signed.
3. Затем на клиенте: сменить `ARTEKI_BASE_URL` на домен, обновить/убрать
   `ARTEKI_PINNED_SPKI`, убрать ATS-исключение в `Info.plist`; на сервере включить
